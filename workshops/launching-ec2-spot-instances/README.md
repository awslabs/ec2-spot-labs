# Launching Amazon EC2 Spot Instances: Workshop guide
...
This workshop is designed to get you familiar with EC2 Spot Instances by walking you through creating an EC2 Launch Template, and then using this Launch Template to launch EC2 Spot Instances the following 4 ways: the EC2 RunInstances API, Amazon EC2 Spot Fleet, Amazon EC2 Fleet, and Amazon EC2 Auto Scaling.

## Requirements and notes
To complete this workshop, have the [AWS CLI](https://aws.amazon.com/cli/) installed, updated to the latest version, and configured.
...
> **Note**: This workshop has been designed to run in the AWS Region of your choice.
...
1. You'll need to gather the following data
	...
	1. AMI ID: Specify an AMI ID from which to launch the instance. You can use an AMI that you own, or you can simply use the Amazon Linux AMI.
	
	1. Instance type: Choose the instance type. Ensure that the instance type is compatible with the AMI you've specified. For more information, see Instance Types.
	
	1.	Subnet: Specify the subnet in which to create a new network interface. For the primary network interface (eth0), this is the subnet in which the instance is launched.
...
$ aws ec2 create-launch-template --launch-template-name TemplateForSpot --version-description TemplateForSpotVersion1 --launch-template-data "{\"NetworkInterfaces\":[{\"DeviceIndex\":0,\"SubnetId\":\"subnet-05ef7d72\"}],\"ImageId\":\"ami-97785bed\",\"InstanceType\":\"c4.large\",\"TagSpecifications\":[{\"ResourceType\":\"instance\",\"Tags\":[{\"Key\":\"Name\",\"Value\":\"EC2SpotImmersionDay\"}]}]}"
	```
	> **Note**: Be sure to replace the ImageId and SubnetId values.
