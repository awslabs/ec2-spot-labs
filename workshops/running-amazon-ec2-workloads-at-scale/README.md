...
11. $ git clone https://github.com/phanan/koel.git
12. move codedeploy structure in place
13. $ aws deploy create-application --application-name koel
14. $ aws deploy push --application-name koel --s3-location s3://cmp402-codedeploybucket-recrh13edl4r/koel.zip --ignore-hidden-files
15. aws deploy create-deployment-group --application-name koel --deployment-group-name runningAmazonEC2WorkloadsAtScale --deployment-config-name CodeDeployDefault.OneAtATime --ec2-tag-filters Key=Name,Value=runningAmazonEC2WorkloadsAtScale,Type=KEY_AND_VALUE --service-role-arn arn:aws:iam::753949184587:role/cmp402-codeDeployServiceRole-1REL37OJOS88N
16. aws deploy create-deployment --application-name koel --deployment-config-name CodeDeployDefault.OneAtATime --deployment-group-name runningAmazonEC2WorkloadsAtScale --s3-location bucket=cmp402-codedeploybucket-recrh13edl4r,bundleType=zip,key=koel.zip
