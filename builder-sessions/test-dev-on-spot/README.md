...
* a CloudWatch alarm that will trigger on 1 minute of 10% errors from the test script
...
The SNS topic created in this stack will be named ReInvent-EC2SpotTestDev-TestFailureAlarmTopic. CloudWatch will publish to this topic if it sees more than 10% error metrics for 1 minute. Feel free to add a subscription to this topic if you'd like to test this alarming (e.g. email or SMS).
...
### 1. Interruption handling via CloudWatch events

Create a CloudWatch event rule to respond to interruptions. In our example [CloudFormation template](interruption_handling/cloudwatch_event_rule.yaml) we'll create an SNS topic, but this is flexible and can even be configured to trigger a Lambda function.

```
aws cloudformation create-stack \
  --template-body file://interruption_handling/cloudwatch_event_rule.yaml \
  --capabilities CAPABILITY_IAM \
  --stack-name ReInvent-EC2SpotTestDev-EventRule
```

Wait for the stack creation to complete (StackStatus == CREATE_COMPLETE).
...
Add a subscription to this SNS topic (e.g. email or SMS) in the AWS Console at Services > SimpleNotificationService > Topics > ReInvent-EC2SpotTestDev-SpotInterruptionTopic > Actions > Subscribe to topic.
...
### 2. Interruption notice on the instance metadata
